...
export async function GET(req: NextRequest) {
  ...
  const max = Number(req.nextUrl.searchParams.get("max") ?? "30");

  try {
    const items = await listGoogleContacts(Number.isFinite(max) ? Math.max(1, Math.min(max, 200)) : 30);
    return NextResponse.json({ total: items.length, items });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown contacts error";
